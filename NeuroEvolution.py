...
class NeuroEvolution:
    ...
    #settings variables:
    popSize = 150 #the number of blips per generation
    mutationRate = .05 #the percent of weights that are changed when mutating (in checkGeneration())
    ...
    def __init__(self):
        ...
        while not self.complete:
            for event in pg.event.get(): #if the X is clicked the game exits
                if event.type == pg.QUIT:
                    self.complete = True
            clock.tick(self.frameRate) #for framerate
            self.checkKeyPress() #looks for key presses
            pg.display.update()
            screen.fill((255,255,255))
            if not self.paused: #these processes happen if the game is not paused
                # loop functions and processes
            ...
            if self.paused: #only the interface is shown if the game is paused
                self.showInterface() #shows text relays, buttons, and the speed slider
        pg.quit()
    ...
    def checkGeneration(self): #controlls adding new, more optomized generations
        if self.generationNumber == 0: #at the beginning of the game, add some random blips
            ...
            if len(self.deadBlips) == self.popSize:#
                self.activeBlips = []#
                self.bars = []
                self.barTimer = 100
    ...
    def checkKeyPress(self): # listens for keypresses
        keys = pg.key.get_pressed()
        if keys[pg.K_SPACE]: #pause/unpause when spacebar is pressed
            if not self.paused: self.paused = True
            else: self.paused = False
            time.sleep(.2) #sleep to avoid rapid pauing and unpausing

    #User Interface Functions:
    def showInterface(self): #shows UI (buttons, slider, and text relays) when paused
        # Text Relays
        ...
        if 970 > mPos[1] > 920: #All buttons have same y-coords so this checks the threshold for all
            if 200 > mPos[0] > 50 and mClick[0] == 1: #restart with a new population at generation 1
